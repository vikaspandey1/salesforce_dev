trigger UpdateIntegrationRealTimeMethodology on Integration_Real_Time__c (before update, after update, before insert, after insert) {
    ...
    if (Trigger.isAfter) {
        ...
        If(Trigger.isUpdate){
            //if(checkRecursive.runOnce()){
                ...
                List<Integration_Real_Time__c> lstObjToUpdate = new List<Integration_Real_Time__c>();
                Set<ID> ids = Trigger.newMap.keySet();
				List<Integration_Real_Time__c> lstCangedConfiguration = [SELECT Id, Methodology__C FROM Integration_Real_Time__c WHERE Id in :ids];
                for(Integration_Real_Time__c chCon : lstCangedConfiguration){
                ...
                if (lstObjToUpdate.isEmpty() == false) {
                    ...
                    Integration_Real_Time__c obj;
                    
                    List<Integration_Real_Time__c> lstObjToBeInserted = new List<Integration_Real_Time__c>();
                    List<Integration_Real_Time__c> lstObjToBeDeleted = new List<Integration_Real_Time__c>();
                    
                    List<Integration_Real_Time__c> lstUpdatedConfiguration = [SELECT Id, Name, 
                                                                      ...
                                                                      Estimate__r.id
                                                                      FROM Integration_Real_Time__c WHERE Id in :lstObjToUpdate];
                    
                    for(Integration_Real_Time__c con : lstUpdatedConfiguration){
                        ...
                        if(strOfferingCapabilityConfigurationMethodology != con.Estimating_Factor__r.Estimating_Factor_ExternalID__c){
                            ...
                            if(lstEstimate !=null && lstEstimate.size() > 0 && lstEstimatingFactor !=null && lstEstimatingFactor.size() > 0 ){
                                ...
                                obj = new Integration_Real_Time__c();
