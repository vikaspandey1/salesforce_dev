trigger UpdateDataMigrationMethodology on Data_Migration__c (before update, after update, before insert, after insert) {
    ...
    if (Trigger.isAfter) {
        ...
        If(Trigger.isUpdate){
            //if(checkRecursive.runOnce()){
                ...
                List<Data_Migration__c> lstObjToUpdate = new List<Data_Migration__c>();
                Set<ID> ids = Trigger.newMap.keySet();
				List<Data_Migration__c> lstCangedConfiguration = [SELECT Id, Methodology__C FROM Data_Migration__c WHERE Id in :ids];
                for(Data_Migration__c chCon : lstCangedConfiguration){
                ...
                if (lstObjToUpdate.isEmpty() == false) {
                    ...
                    Data_Migration__c obj;
                    
                    List<Data_Migration__c> lstObjToBeInserted = new List<Data_Migration__c>();
                    List<Data_Migration__c> lstObjToBeDeleted = new List<Data_Migration__c>();
                    
                    List<Data_Migration__c> lstUpdatedConfiguration = [SELECT Id, Name, 
                                                                      ...
                                                                      Estimate__r.id
                                                                      FROM Data_Migration__c WHERE Id in :lstObjToUpdate];
                    
                    for(Data_Migration__c con : lstUpdatedConfiguration){
