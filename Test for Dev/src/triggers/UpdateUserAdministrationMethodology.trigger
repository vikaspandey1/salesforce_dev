trigger UpdateUserAdministrationMethodology on User_Administration__c (before update, after update, before insert, after insert) {
    ...
    if (Trigger.isAfter) {
        ...
        If(Trigger.isUpdate){
            //if(checkRecursive.runOnce()){
                ...
                List<User_Administration__c> lstObjToUpdate = new List<User_Administration__c>();
                Set<ID> ids = Trigger.newMap.keySet();
				List<User_Administration__c> lstCangedConfiguration = [SELECT Id, Methodology__C FROM User_Administration__c WHERE Id in :ids];
                for(User_Administration__c chCon : lstCangedConfiguration){
                ...
                if (lstObjToUpdate.isEmpty() == false) {
                    ...
                    User_Administration__c obj;
                    
                    List<User_Administration__c> lstObjToBeInserted = new List<User_Administration__c>();
                    List<User_Administration__c> lstObjToBeDeleted = new List<User_Administration__c>();
                    
                    List<User_Administration__c> lstUpdatedConfiguration = [SELECT Id, Name, 
                                                                      ...
                                                                      Estimate__r.id
                                                                      FROM User_Administration__c WHERE Id in :lstObjToUpdate];
                    
                    for(User_Administration__c con : lstUpdatedConfiguration){
                        ...
                        if(strOfferingCapabilityConfigurationMethodology != con.Estimating_Factor__r.Estimating_Factor_ExternalID__c){
                            ...
                            if(lstEstimate !=null && lstEstimate.size() > 0 && lstEstimatingFactor !=null && lstEstimatingFactor.size() > 0 ){
                                ...
                                obj = new User_Administration__c();
