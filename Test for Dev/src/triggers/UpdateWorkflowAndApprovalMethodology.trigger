trigger UpdateWorkflowAndApprovalMethodology on Workflow_and_Approval__c (before update, after update, before insert, after insert) {
    ...
    if (Trigger.isAfter) {
        ...
        If(Trigger.isUpdate){
            //if(checkRecursive.runOnce()){
                ...
                List<Workflow_and_Approval__c> lstObjToUpdate = new List<Workflow_and_Approval__c>();
                Set<ID> ids = Trigger.newMap.keySet();
				List<Workflow_and_Approval__c> lstCangedConfiguration = [SELECT Id, Methodology__C FROM Workflow_and_Approval__c WHERE Id in :ids];
                for(Workflow_and_Approval__c chCon : lstCangedConfiguration){
                ...
                if (lstObjToUpdate.isEmpty() == false) {
                    ...
                    Workflow_and_Approval__c obj;
                    
                    List<Workflow_and_Approval__c> lstObjToBeInserted = new List<Workflow_and_Approval__c>();
                    List<Workflow_and_Approval__c> lstObjToBeDeleted = new List<Workflow_and_Approval__c>();
                    
                    List<Workflow_and_Approval__c> lstUpdatedConfiguration = [SELECT Id, Name, 
                                                                      ...
                                                                      Estimate__r.id
                                                                      FROM Workflow_and_Approval__c WHERE Id in :lstObjToUpdate];
                    
                    for(Workflow_and_Approval__c con : lstUpdatedConfiguration){
                        ...
                        if(strOfferingCapabilityConfigurationMethodology != con.Estimating_Factor__r.Estimating_Factor_ExternalID__c){
                            ...
                            if(lstEstimate !=null && lstEstimate.size() > 0 && lstEstimatingFactor !=null && lstEstimatingFactor.size() > 0 ){
                                ...
                                obj = new Workflow_and_Approval__c();
