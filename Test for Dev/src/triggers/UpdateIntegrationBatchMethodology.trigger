trigger UpdateIntegrationBatchMethodology on Integration_Batch__c (before update, after update, before insert, after insert) {
    ...
    if (Trigger.isAfter) {
        ...
        If(Trigger.isUpdate){
            //if(checkRecursive.runOnce()){
                ...
                List<Integration_Batch__c> lstObjToUpdate = new List<Integration_Batch__c>();
                Set<ID> ids = Trigger.newMap.keySet();
				List<Integration_Batch__c> lstCangedConfiguration = [SELECT Id, Methodology__C FROM Integration_Batch__c WHERE Id in :ids];
                for(Integration_Batch__c chCon : lstCangedConfiguration){
                ...
                if (lstObjToUpdate.isEmpty() == false) {
                    ...
                    Integration_Batch__c obj;
                    
                    List<Integration_Batch__c> lstObjToBeInserted = new List<Integration_Batch__c>();
                    List<Integration_Batch__c> lstObjToBeDeleted = new List<Integration_Batch__c>();
                    
                    List<Integration_Batch__c> lstUpdatedConfiguration = [SELECT Id, Name, 
                                                                      ...
                                                                      Estimate__r.id
                                                                      FROM Integration_Batch__c WHERE Id in :lstObjToUpdate];
                    
                    for(Integration_Batch__c con : lstUpdatedConfiguration){
